...
namespace UnitTestScribrAPI
{
    ...
    public class TranscriptionsControllerUnitTests
    {
        ...
        public async Task TestPutTranscriptionNoContentStatus()
        {
            using (var context = new scriberContext(options))
            {
                string phrase = "this is now a different prase ";
                ...
                TranscriptionsController transcirptionController = new TranscriptionsController(context);
                IActionResult result = await transcirptionController.PutTranscription(transcription1.TranscriptionId, transcription1) as IActionResult;
